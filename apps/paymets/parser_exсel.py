...
def ensure_xlsx(input_path: str) -> Worksheet:
...
def kazpost_extract_payment_data(sheet: Worksheet):
    """
    ...
    for row in sheet.iter_rows(min_row=header_row_idx + 1, values_only=True):
        ...
        results.append({
            "Дата": document_date,
            "Лицевой счет": acc_str,
            "Идентификатор платежа": op_str,
            "Сумма платежа": amount_f,
            "Источник": "Казпочта"
        })
...
def kaspi_extract_payment_data(sheet: Worksheet):
    """
    ...
    Ожидаемая структура:
      Строка 1: заголовок отчета (пропускается)
      Строка 2: шапка колонок: Дата | Идентификатор платежа | Лицевой счет | Сумма платежа
      Далее: строки с данными, затем блоки 'Общая сумма', 'Комиссия', 'Сумма к перечислению', 'Количество'
    """
    ...
    for row in sheet.iter_rows(min_row=header_row_idx + 1, values_only=True):
        ...
        if payment_id_val is not None:
            payment_id_out = str(int(payment_id_val)) if isinstance(payment_id_val, (int, float)) else str(
                payment_id_val)
        ...
        results.append({
            "Дата": date_out,
            "Лицевой счет": account_out,
            "Идентификатор платежа": payment_id_out,
            "Сумма платежа": amount_out,
            "Исчтоник" : "Kaspi"
        })
...
def halyk_extract_payment_data(sheet: Worksheet):
    """
    ...
    Ожидаемая структура:
      Строка 1: заголовок отчета "РЕЕСТР ПЛАТЕЖЕЙ..."
      Строка 2: шапка колонок: Дата операционного дня | Идентификатор платежа | Лицевой счет | Сумма платежа | ...
      Далее: строки с данными, затем блоки итогов
    """
...
def bcc_extract_payment_data(sheet: Worksheet):
...
if __name__ == "__main__":
    sheet = ensure_xlsx("reestr251017.xls")
    data = bcc_extract_payment_data(sheet)
    print(data)
